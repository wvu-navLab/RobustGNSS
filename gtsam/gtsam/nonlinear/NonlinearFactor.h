 ...
 */
template<class VALUE>
class pseudoRangeFactor: public NoiseModelFactor {
...
protected:
  ...
  typedef NoiseModelFactor Base;
  typedef pseudoRangeFactor<VALUE> This;
...
public:
   ...
   *  @param key1 by which to look up X value in Values
   */
  pseudoRangeFactor(const SharedNoiseModel& noiseModel, Key key1, double measured, Vector gnssKnowns) :
    Base(noiseModel, cref_list_of<1>(key1)) {}
