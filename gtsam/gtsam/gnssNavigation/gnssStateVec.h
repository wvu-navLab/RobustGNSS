...
class GTSAM_EXPORT gnssStateVec : public Vector5  {
  ...
  explicit gnssStateVec(const Vector5& v) :
    ...
    gnssStateVec retract(const Vector5& v) const { return compose(gnssStateVec(v));}
    static Vector5 Logmap(const gnssStateVec& p) { return p;}
    static gnssStateVec Expmap(const Vector5& v) { return gnssStateVec(v);}
