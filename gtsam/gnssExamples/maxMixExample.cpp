...
int main(int argc, char** argv) {
        ...
        double measWeight, switchInit, switchPrior, weight, percentFaulty;
        double residualThresh, processScale(1.0), timeStep(0.1), prevTime;
        ...
        for(unsigned int i = 0; i < outlierKeys.size(); i++ ) {
                ...
                PseudorangeMaxMix maxMixFactor(X(currKey), (range - rho),
                                               obsMap(satXYZ, nomXYZ, trop), hypothesis, null_model, measWeight, weight);

                graph.remove(factorIndex);
                graph.add(maxMixFactor);
        }
        // Optimize the graph
